...
from invitationapp.forms import UserCreationForm,InvitationForm
        ...
        if form.is_valid():
            ...
            if invitor_exist:
                for i in invitor_exist:
                    ...
                    send_mail('Invitaion',str(i.invitee_email) + ' has accepted your invitation and registered','pavan1995143.pavan@gmail.com',[str(i.invitor_email),])
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                form.save()
                send_mail('Invitaion', str(request.user) +' invited you to register the app: http://127.0.0.1:8000/invitations/signup/ ','pavan1995143.pavan@gmail.com',[form.invitee_email,])
                send_mail('Invitaion', 'Invition has been sent to '+form.invitee_email,'pavan1995143.pavan@gmail.com',[form.invitor_email,])
            return HttpResponseRedirect(reverse('invitationapp:index')) 
    else:
       return HttpResponseRedirect(reverse('invitationapp:login')) 